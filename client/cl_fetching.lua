...
fetching.execute = function(name, ...)
    local callId, solved = callIden, false
    callIden = callIden + 1

    promises[callId] = promise:new()
    TriggerServerEvent('fetching:cl_request', res, name, callId, paramPacker(...))
...
RegisterNetEvent('fetching:cl_response')
AddEventHandler('fetching:cl_response', function(origin, callId, response)
